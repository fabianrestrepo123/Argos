...
test("Eliminación de múltiples productos deja el carrito vacío", async ({ page }) => {
  ...
  await page.reload();
  const totalEsperado =
    precioDetalleMonitores + precioDetalleCelulares + precioDetalleLaptops;

  await productos.irAlCarrito();
  await carrito.eliminarProducto("Apple monitor 24");
  await carrito.eliminarProducto("Nexus 6");
  await carrito.eliminarProducto("MacBook air");

  await carrito.validarCarritoVacio();
});
